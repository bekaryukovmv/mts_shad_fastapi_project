...
@pytest.mark.asyncio
async def test_create_book(async_authenticated_client):
    data = {"title": "Wrong Code",
            "author": "Robert Martin",
            "pages": 104,
            "year": 2007}
    response = await async_authenticated_client.post("/api/v1/books/", json=data)
...
@pytest.mark.asyncio
async def test_get_books(db_session, async_authenticated_client):
    ...
    response = await async_authenticated_client.get("/api/v1/books/")
...
@pytest.mark.asyncio
async def test_get_single_book(db_session: AsyncSession, async_authenticated_client):
    # Создаем книги вручную, а не через ручку, чтобы нам не попасться на ошибку которая
    # может случиться в POST ручке
    book = books.Book(author="Pushkin", title="Eugeny Onegin", year=2001, count_pages=104, seller_id=1)
    book_2 = books.Book(author="Lermontov", title="Mziri", year=1997, count_pages=104, seller_id=1)
    ...
    response = await async_authenticated_client.get(f"/api/v1/books/{book.id}")
...
@pytest.mark.asyncio
async def test_update_book(db_session, async_authenticated_client):
    # Создаем книги вручную, а не через ручку, чтобы нам не попасться на ошибку которая
    # может случиться в POST ручке
    book = books.Book(author="Pushkin", title="Eugeny Onegin", year=2001, count_pages=104)
